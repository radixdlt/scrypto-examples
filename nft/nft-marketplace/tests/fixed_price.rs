...
pub static BLUEPRINT_NAME: &str = "FixedPriceSale";
pub static INSTANTIATION_FUNCTION_NAME: &str = "instantiate_fixed_price_sale";
...
pub fn full_run_with_price_change_succeeds(){
    ...
    let purchase_tx: SignedTransaction = TransactionBuilder::new()
        ...
        .sign([&env.accounts[0].private_key]);
    let purchase_receipt: Receipt = env.executor.validate_and_execute(&purchase_tx).unwrap();
    assert!(purchase_receipt.result.is_err(), "Buying NFTs failed");
}