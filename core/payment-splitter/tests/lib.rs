...
fn admin_can_add_shareholder() {
    ...
    ): (EcdsaPublicKey, EcdsaPrivateKey, ComponentAddress) = test_runner.new_account();
    let (
    ...
    let instantiation_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        .call_function(package_address, "PaymentSplitter", "instantiate_payment_splitter", to_struct!(RADIX_TOKEN))
        .call_method_with_all_resources(admin_component_address, "deposit_batch")
        .build();
    let instantiation_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(instantiation_tx, vec![admin_public_key]);
    ...
    let adding_shareholder_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        .create_proof_from_account_by_amount(dec!("1"), admin_badge_resource_address, admin_component_address)
        .call_method(payment_splitter_component_address, "add_shareholder", to_struct!(dec!("20")))
        .call_method_with_all_resources(non_admin_component_address, "deposit_batch")
        .build();
    let adding_shareholder_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(adding_shareholder_tx, vec![admin_public_key]);
...
fn shareholder_cant_add_shareholder() {
    ...
    // Attempting to add a new shareholder to the payment splitter
    let unauthed_adding_shareholder_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        .create_proof_from_account_by_amount(dec!("1"), shareholder_badge_resource_address, non_admin_component_address)
        .call_method(payment_splitter_component_address, "add_shareholder", to_struct!(dec!("999999")))
        .call_method_with_all_resources(non_admin_component_address, "deposit_batch")
        .build();
    let unauthed_adding_shareholder_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(unauthed_adding_shareholder_tx, vec![admin_public_key]);
    ...
    unauthed_adding_shareholder_receipt.expect_failure(|error: &RuntimeError| -> bool {
        matches!(error, RuntimeError::AuthorizationError{function: _, authorization: _, error: _})
    });
}
...
fn unauthed_cant_lock_splitter() {
    ...
    // Attempting to add a new shareholder to the payment splitter
    let unauthed_locking_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        .call_method(payment_splitter_component_address, "lock_splitter", to_struct!())
        .build();
    let unauthed_locking_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(unauthed_locking_tx, vec![admin_public_key]);
...
fn admin_cant_add_shareholder_after_locking() {
    ...
    let locking_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        .create_proof_from_account_by_amount(dec!("1"), admin_badge_resource_address, admin_component_address)
        .call_method(payment_splitter_component_address, "lock_splitter", to_struct!())
        .build();
    let _locking_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(locking_tx, vec![admin_public_key]);
...
fn anybody_can_deposit() {
    ...
    let deposit_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        ...
        .call_method_with_all_resources(non_admin_component_address, "deposit_batch")
        .build();
    let deposit_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(deposit_tx, vec![non_admin_public_key]);
...
fn custom_rule_splitter_works_with_correct_badges() {
    ...
    let badge_creation_tx: TransactionManifest = ManifestBuilder::new(Network::LocalSimulator)
        .new_badge_fixed(HashMap::new(), dec!("1"))
        .new_badge_fixed(HashMap::new(), dec!("1"))
        .new_badge_fixed(HashMap::new(), dec!("1"))
        .call_method_with_all_resources(admin_component_address, "deposit_batch")
        .build();
    let badge_creation_receipt: TransactionReceipt = test_runner.execute_manifest_ignoring_fee(badge_creation_tx, vec![admin_public_key]);
